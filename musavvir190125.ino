...
bool sendToSupabase(const char* table, const String& jsonData, const String& method = "POST");
  ...
  htmlPage += R"rawliteral(
        ...
        <script>
            function editTeacher(id, name, courseName, courseCode) {
                ...
                if (newName && newCourseName && newCourseCode) { // Fixed typo
...
void handleUpdateTeacher() {
    ...
    while (oldFile.available()) {
        ...
        if (line.startsWith(id + ",")) {
            ...
            // Prepare JSON data for Supabase (adjust the keys to match your table schema)
            StaticJsonDocument<300> doc;
            doc["teacher_id"] = id.toInt(); // Ensure teacher_id is passed as an integer
            doc["name"] = newName;
            doc["course_name"] = newCourseName;
            doc["course_code"] = newCourseCode;

            String jsonString;
            serializeJson(doc, jsonString);

            // Debug: Log JSON data being sent
            Serial.println("Sending JSON to Supabase:");
            Serial.println(jsonString);

            // Send update to Supabase
            if (!sendToSupabase("teachers", jsonString, "PATCH")) {
                Serial.println("Failed to update Supabase for teacher ID: " + id);
            } else {
                Serial.println("Successfully updated Supabase for teacher ID: " + id);
            }
        } else {
...
void handleDeleteTeacher() {
    ...
    while (oldFile.available()) {
        ...
        if (!line.startsWith(id + ",")) {
            ...
            // Prepare JSON for Supabase deletion
            StaticJsonDocument<300> doc;
            doc["teacher_id"] = id.toInt();
            String jsonString;
            serializeJson(doc, jsonString);
            sendToSupabase("teachers", jsonString, "DELETE"); // Delete from Supabase
        }
    }
...
void handleUpdateStudent() {
    ...
    while (oldFile.available()) {
        ...
        if (line.startsWith(id + ",")) {
            ...
            doc["student_id"] = id.toInt();
            doc["name"] = newName;
            doc["roll_number"] = newRollNumber;
            String jsonString;
            serializeJson(doc, jsonString);
            sendToSupabase("students", jsonString); // Update Supabase
        } else {
            newFile.println(line);
        }
    }
...
void handleDeleteStudent() {
    ...
    while (oldFile.available()) {
        ...
        if (!line.startsWith(id + ",")) {
            ...
            String jsonString;
            serializeJson(doc, jsonString);
            sendToSupabase("students", jsonString, "DELETE"); // Delete from Supabase
        }
    }
...
void handleDeleteAttendanceRecord() {
    ...
    while (oldFile.available()) {
        ...
        if (!line.startsWith(courseCode + "," + studentId + "," + timestamp)) {
            ...
            String jsonString;
            serializeJson(doc, jsonString);
            sendToSupabase("attendance", jsonString, "DELETE"); // Delete from Supabase
        }
    }
    ...
    SPIFFS.rename("/temp.csv", attendanceDataFile);
...
bool sendToSupabase(const char* table, const String& jsonData, const String& method) {
    HTTPClient http;
    ...
    if (method == "DELETE") {
        ...
        httpCode = http.POST(jsonData); // Send POST request
    }

    // Debug: Log the response code and body
    String response = http.getString();
    Serial.println("HTTP Code: " + String(httpCode));
    Serial.println("Response: " + response);

    bool success = (httpCode == 200 || httpCode == 201); // Check for success
    http.end();
...
bool checkSerialExists(const String& serial) {
    ...
    if (httpCode == 200) {
        ...
        Serial.println("Error checking serial: " + String(httpCode));
    }
...
bool updateIPAddress(const String& serial, const String& ip) {
    ...
    int httpCode = http.PATCH(jsonString); // Use PATCH to update

    bool success = (httpCode == 200 || httpCode == 204); // Check for success
    http.end();
    return success;
}
bool connectToWiFi(const char* ssid, const char* password) {
    ...
    String serial = "1"; // Your unique serial number
    ...
    if (checkSerialExists(serial)) {
        ...
        // Prepare to send the new IP address to Supabase
        StaticJsonDocument<200> doc;
...
void startAccessPoint() {
    ...
    lcd.print("AP IP"); // Display Wi-Fi failed message
...
void handleSaveWifi() {
    ...
    // Redirect to fb.com
    server.sendHeader("Location", "http://www.fb.com");
    server.send(303); // 303 See Other

    // Delay to allow the user to see the redirection
...
void saveAttendance(String coursecode, String studentName, int studentId) {
 ...
 char timestamp[32];
 strftime(timestamp, sizeof(timestamp), "%Y-%m-%d", localTime);
    // Save to SD Card
...
void startAttendanceProcess() {
    ...
    // Wait until the correct teacher fingerprint is provided
    while (true) {
        if (verifyTeacherFingerprint(coursecode)) {
            lcd.clear();
            lcd.setCursor(0, 0);
    ...
    if (attendanceFile) {
        while (attendanceFile.available()) {
            ...
            if (firstComma != -1 && lastComma != -1) {
                ...
                String date = line.substring(lastComma + 1, lastComma + 11);
    ...
    while (oldFile.available()) {
        ...
        if (line.length() > 0) {
            ...
            if (existingCourseCode == courseCode &&
                ...
                doc["timestamp"] = newTimestamp;
                String jsonString;
                serializeJson(doc, jsonString);
                sendToSupabase("attendance", jsonString); // Update Supabase
            } else {
...
void handleDownloadDateAttendance() {
   ...
   // Get course name
    ...
    // Write CSV header with course name
    ...
    if (attendanceFile) {
        while (attendanceFile.available()) {
            ...
            if (line.startsWith(courseCode + ",") && line.indexOf(date) != -1) {
                // Add course name to the beginning of each line
...
void setup() {
  ...
  server.begin();
if (!SD.begin(33)) { // CS pin connected to GPIO 33
